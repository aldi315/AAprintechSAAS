...
export function MusicPlayer({ data, props }: SectionComponentProps) {
  const audioRef = useRef<HTMLAudioElement | null>(null)
  const [isPlaying, setIsPlaying] = useState(false)
  const musicUrl = (props.musicUrl as string) || data.musicUrl
  ...
  return (
    ...
    >
      ...
      {isPlaying && (
        <span className="absolute inset-0 rounded-full animate-ping opacity-30" style={{ backgroundColor: themeColor }} />
      )}
      {/* Icon */}
      <svg
